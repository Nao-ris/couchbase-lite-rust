...
#[test]
fn start_stop_delete_db_start() {
    let tmp_dir = TempDir::new("cbl_rust").expect("create temp dir");

    // To see couchbase-lite-C logs
    //init_logging();
    ...
    let repl_config = ReplicatorConfiguration {
        database: local_database.as_ref().unwrap().clone(),
        endpoint: Endpoint::new_with_local_db(&central_database),
    ...
    let repl_context = Box::new(ReplicationConfigurationContext {
        push_filter: None,
        pull_filter: None,
    ...
    let mut replicator = Some(Replicator::new(repl_config, repl_context).unwrap());
    ...
    // Stop replicator
    assert!(replicator.as_mut().unwrap().stop(Some(20)));

    // Release replicator
    replicator = None;

    println!("=== Replicator stopped & dropped ===");
    std::thread::sleep(Duration::from_secs(2));

    // Release database handle
    local_database = None;

    println!("=== Database instance dropped ===");
    std::thread::sleep(Duration::from_secs(2));

    // Delete database file without using CBL_DeleteDatabase or CBLDatabase_Delete, directly with the file system
    std::fs::remove_dir_all(tmp_dir.path().to_path_buf().join(&format!("local.cblite2"))).unwrap();
    ...
    replicator = Some(Replicator::new(repl_config, repl_context).unwrap());
    replicator.as_mut().unwrap().start(false);
    ...
    if could_stop_replicator {
        println!("=== Replicator stopped ===");
    } else {
        println!("=== Replicator could not be stopped ===");
    }

    // Uncommenting this section will make the test hang indefinitely
    /*
    replicator = None;

    println!("=== Hanging replicator stopped & dropped ===");
    std::thread::sleep(Duration::from_secs(2));
    ...
    replicator = Some(Replicator::new(repl_config, repl_context).unwrap());
    println!("=== Printed: New replicator ===");
    replicator.as_mut().unwrap().start(false);
    println!("=== Never printed, CBLReplicator_Start hangs indefinitely: non-continuous replicator started ===");
    */

    // All replicator & database handles will be released automatically
}
